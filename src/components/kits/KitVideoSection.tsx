...
export default function KitVideoSection({
  kitId,
  videoName,
  videoScript,
  videoYoutubeUrl,
  ...
  const upload = async (file: File) => {
    ...
    if (!kitId) {
      ...
      return toast.error("Salve o kit antes de enviar vídeo.");
    }
    setUploading(true);
    try {
      ...
      const path = `${userId}/${kitId}/${Date.now()}-${safe}`;
      ...
      <div className="border-t border-sidebar-border/30 pt-4 space-y-3">
        ...
        {!videoPath && !pendingFile && (
          ...
          >
            ...
            {!kitId && !onPendingFileChange && (
              <p className="text-[11px] text-amber-500 mt-2">
                Salve o kit primeiro para habilitar o upload.
              </p>
            )}
          </div>
        )}
        {pendingFile && pendingPreview && (
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-amber-500">
              Vídeo na fila — será enviado ao salvar o kit.
            </p>
            <button
              type="button"
              onClick={removeVideo}
