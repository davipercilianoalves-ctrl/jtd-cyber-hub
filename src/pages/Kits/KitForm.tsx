...
export default function KitForm({ kitId }: KitFormProps) {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(!!kitId);
  ...
  async function handleSubmit(e?: React.FormEvent) {
    ...
    if (!formData.name?.trim()) {
      toast.error("Preencha o nome do kit para continuar");
      return;
    }
    ...
    try {
      ...
      if (savedKitId) {
        const { error: delErr } = await supabase.from("kit_competitors").delete().eq("kit_id", savedKitId);
      ...
      toast.error("Não foi possível salvar o kit", {
        description: formatSupabaseError(error, "Erro ao salvar kit."),
      });
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-12 animate-in fade-in duration-300">
      ...
      <section className="jtd-glass p-6 space-y-6 relative">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-x-6 gap-y-4">
            <Field label="Nome do Kit*" value={formData.name} span={3}>
              <input
                required
                value={formData.name}
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  onClick={() =>
                    setFormData({
                      ...formData,
                      sku: `KIT-${Math.random().toString(36).substr(2, 5).toUpperCase()}`,
                    })
                  }
      ...
      <div className="space-y-3 pt-2">
        ...
        <h3 className="text-sm font-semibold">Imagens do Kit</h3>
        {kitId ? (
          <KitImageGallery kitId={kitId} />
        ) : (
          <div className="space-y-3">
            <input
              ref={pendingFileRef}
            ...
            <div className="border-2 border-dashed border-sidebar-border rounded-lg p-6 text-center bg-internal-20">
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Selecione as imagens agora — elas serão enviadas automaticamente após salvar o kit.
                </p>
                <button
                  type="button"
